...
std::vector<double> Dantzig(const std::vector<double>& v, const std::vector<double>& w,
                            const std::vector<double>& u, double C) {
  ...
  std::stable_sort(order.begin(), order.end(),
                   [&](std::size_t a, std::size_t b) { return v[a] * w[b] > v[b] * w[a]; });
...
double Value(const std::vector<double>& v, const std::vector<double>& z) {
  if (v.size() != z.size()) throw std::invalid_argument("Value: v and z must have the same length");
  double total = 0.0;
